...
public class MenuNode implements Serializable, Iterable<MenuNode> {
    ...
    public void build(Element tree){
        ...
        String tab = tree.getAttributeValue("tab");
        System.out.println("PermsAsString : "+permsAsString+", tab : "+tab);
        DB db = new DB();
        db.open();
        ...
        if(module!=null){
        	permsAsString="";
        	for(Moduleperm moduleperm:module.getModuleperm()){
        		permsAsString+=!permsAsString.equalsIgnoreCase("")&&permsAsString.length()>0?" ":"";
        		permsAsString+=moduleperm.getRole().getName().trim();
        	}
        }
        System.out.println("PermsAsString : "+permsAsString+", after Module ");
